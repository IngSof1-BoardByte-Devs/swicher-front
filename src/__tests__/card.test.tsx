...
describe('fetch_figure_cards', () => {
    ...
    it('should throw an error when the server response is not successful', async () => {
        fetchMock.mockRejectedValueOnce(
            new Error('An error occurred while fetching the figure cards'),
        );
...
describe('GetMoveCard', () => {
    ...
    test('should return success when the fetch request is successful', async () => {
        const mockResponse = {
            status: 'OK',
            message: 'Movement cards fetched successfully',
        };
        fetchMock.mockResponseOnce(JSON.stringify(mockResponse), {
            status: 200,
        });
    ...
    test('should return error when the fetch request fails', async () => {
        ...
        fetchMock.mockRejectOnce(
            new Error('An error occurred while getting the movement cards'),
        );
        const result = await fetch_movement_cards({ id_player });
        expect(result).toEqual(mockResponse);
    });
...
describe('use_movement_cards', () => {
    ...
    test('status 200', async () => {
        fetchMock.mockResponse(
            JSON.stringify({ status: 'ERROR', message: 'invalid player id' }),
            { status: 200 },
        );
        ...
        expect(fetch).toHaveBeenCalledWith(
            `http://localhost:8000/movement-cards/${id_card}/`,
            {
                method: 'PATCH',
                headers: {
    ...
    test('status 404', async () => {
        ...
        expect(result?.toString()).toBe(
            'La carta enviada no existe o no se puede usar',
        );
        expect(fetch).toHaveBeenCalledWith(
            `http://localhost:8000/movement-cards/${id_card}/`,
            {
    ...
    test('status 401', async () => {
        ...
        expect(result?.toString()).toBe(
            'No tienes permisos para usar esta carta',
        );
        expect(fetch).toHaveBeenCalledWith(
            `http://localhost:8000/movement-cards/${id_card}/`,
            {
    ...
    test('error while fetching', async () => {
        ...
        if (typeof result === 'object' && 'message' in result) {
            expect(result.message).toBe(
                `Error al intentar usar la carta id: ${id_card}, ${error}`,
            );
        }
        expect(fetch).toHaveBeenCalledWith(
            `http://localhost:8000/movement-cards/${id_card}/`,
