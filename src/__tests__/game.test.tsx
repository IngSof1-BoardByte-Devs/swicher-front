...
describe('create_game function', () => {
    ...
    it('should handle both a successful 200 response and a failure', async () => {
        ...
        const successResult = await create_game({
            player_name: 'John',
            game_name: 'Test Game',
        });
        ...
        fetchMock.mockRejectedValueOnce(
            new Error('An error occurred while creating the game'),
        );

        const failResult = await create_game({
            player_name: 'John',
            game_name: 'Test Game',
        });
...
describe('join_game', () => {
    ...
    it('should return error if player_name is missing', async () => {
        ...
        expect(result).toEqual({
            status: 'ERROR',
            message: 'Nombre de jugador invalido',
        });
...
describe('start_game', () => {
    ...
    test('url should be correct', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith(
            `http://localhost:8000/games/${player_id}`,
            {
                method: 'PUT',
                headers: {
...
describe('cancel movements', () => {
    ...
    test('status 200', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith(
            `http://localhost:8000/games/${game_id}/revert-moves`,
            {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
    ...
    test('status 401', async () => {
        fetchMock.mockResponseOnce(
            JSON.stringify({
                status: 'ERROR',
                detail: 'No tienes permisos para cancelar los movimientos',
            }),
            { status: 401 },
        );
        const result = await revert_movements({ game_id: game_id });
        expect(result.message).toBe(
            'No tienes permisos para cancelar los movimientos',
        );
    });
});
